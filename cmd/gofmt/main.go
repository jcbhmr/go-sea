...
var GoFS = layerfs.New(
	...
	prefixfs.New(filterfs.ExcludePaths(binfs.BinFS, "go"+exeSuffix, "gofmt"+exeSuffix), "pkg/tools/"+runtime.GOOS+"_"+runtime.GOARCH),
)
...
func main() {
	...
	if err != nil {
		if errors.Is(err, fs.ErrNotExist) {
			...
			err = osutil.CopyFSOverwrite(appCacheDir, GoFS)
			if err != nil {
				return
			}
		} else {
	...
	err = crossexec.Exec(filepath.Join(appCacheDir, "bin", "gofmt"+exeSuffix), os.Args, os.Environ())
}
